...
# ---------------------------- #
# ---------------------------- #
logger_request = logging.getLogger('log_request')
...
def BaseStringReturn(request_input, status_code, logging_description):
    ...
    if logging_description.status_write_msg:
        string_log_info = "\"{user}\" {ip} \"{method} {url}\" {status_code} {logging_description}".format(
            user=request_input.user_current.username if request_input.user_current != None else None,
            ip="\"{ip} - {name}\"".format(ip=request_input.META.get(
                "REMOTE_ADDR"), name=socket.gethostname()),
            method=request_input.method,
            url=request_input.get_full_path(),
            status_code=status_code, logging_description=str(logging_description.__str__()))
    else:
        string_log_info = "\"{user}\" {ip} \"{method} {url}\" {status_code} ".format(
            user=request_input.user_current.username if request_input.user_current != None else None,
            ip="\"{ip} - {name}\"".format(ip=request_input.META.get(
                "REMOTE_ADDR"), name=socket.gethostname()),
            method=request_input.method,
...
# ---------------------------- #
# ---------------------------- #
def REQUEST_ExcuteLogging_Info(request_input, status_code, logging_description):
    return logger_request.info(BaseStringReturn(request_input, status_code, logging_description))
