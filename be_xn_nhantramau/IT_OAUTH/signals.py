            ...
            with open(user_fixture_path, 'r', encoding='utf-8') as f:
                ...
                for user_data in users_to_create:
                    ...
                    try:
                        ...
                        if created:
                            print(
                                f"Created Role: '{role_code}' with name '{name}'")
                        if is_superuser:
                            ...
                            user, created_user = User.objects.get_or_create(
                                username=username,
                                defaults={
                                    'email': email, 'is_superuser': False, 'is_staff': False}
                            )
                            if created_user:
                                user.set_password('12345678')
                                user.save()
                        ...
                        if created_user:
                            print(
                                f"Created User: {username} with Role: {name}")
                        else:
                            print(
                                f"Updated User: {username} with Role: {name}")
                    except IntegrityError as e:
                        print(
                            f"Skipping user '{username}' due to IntegrityError: {e}")
        ...
        try:
            with open(config_fixture_path, 'r', encoding='utf-8') as f:
                ...
                for config_data in configs_to_create:
                    ...
                    if not name_config:
                        print(
                            "Warning: Skipping config due to missing 'name_config'.")
                        continue
