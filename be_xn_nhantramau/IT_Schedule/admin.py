...
from .models import Task, Schedule, TaskLog
...
current_app_config = apps.get_app_config('IT_Schedule')
...
def create_model_admin(model):
    ...
    for field in model._meta.get_fields():
        ...
        # Handle regular fields
            ...
            ForeignKey, DateTimeField, model_fields.TextField, model_fields.UUIDField
        )):
    ...
    # Link the dynamically created form to this ModelAdmin class
    admin_class_attributes['form'] = create_optional_model_form(model)

    # --- THIS IS THE CRITICAL CORRECTION ---
    # Define the save_model method here, where it correctly belongs to ModelAdmin
    def save_model(self, request, obj, form, change):
        """
        Override save_model to trigger a re-scheduling of all tasks
        when a change is made to a Schedule object in the admin panel.
        """
        # Call the parent's save_model to save the object to the database
        super(type(self), self).save_model(request, obj, form, change)

        # Now, check if the model is Schedule and call the scheduler
        if model == Schedule:
            try:
                scheduler_manager.reload_schedules()
                print(f"Admins save_model restart")
            except Exception as e:
                print(f"Admins save_model {e}")

    # Add the save_model method to the class attributes
    admin_class_attributes['save_model'] = save_model
    # --- END OF CORRECTION ---
